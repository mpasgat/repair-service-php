...
Короткий сценарий для проверяющего:
1. Создание заявки: открыть `http://89.23.105.215/requests/create` и отправить форму.
2. Вход в роль: открыть `http://89.23.105.215/login` и выбрать пользователя.
3. Проверка диспетчера: выбрать `Диспетчер Анна` и открыть `http://89.23.105.215/dispatcher`.
4. Проверка мастера: выйти, выбрать `Мастер Иван` или `Мастер Петр`, открыть `http://89.23.105.215/master`.

Примечание:
- Регистрация не нужна. Авторизация сделана через выбор пользователя на ` /login `.
